...
2. **Revisar propuesta** — tres grupos:
   ...
   - **Requieren revisión** (`missing` + `review`)
...
- `review`: checkbox manual permitido; aviso en preview.
- `exclude`: sin checkbox; servidor rechaza importación.

## Límites conocidos

- Sin OCR: requiere texto embebido útil en el PDF.
- Soportes y partidas fuera del BOM no aparecen (ver business set 15C).
...
Plano `seed-drawing-e2e-bom`: 18 listas para incluir, 1 revisión, 1 exclusión, 1 ya existente.
